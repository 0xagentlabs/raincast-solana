...
export default function Home() {
  ...
  const [markets, setMarkets] = useState<Market[]>([]); const [positions, setPositions] = useState<Record<string,Position|null>>({}); const [busy, setBusy] = useState(""); const [notice, setNotice] = useState(""); const [noticeSignature, setNoticeSignature] = useState(""); const [oracle, setOracle] = useState<PublicKey>(); const [authority,setAuthority]=useState<PublicKey>(); const [fees,setFees]=useState(0n); const [now, setNow] = useState(() => Math.floor(Date.now()/1000));
  const refresh = useCallback(async () => { const [rows, configured, rent] = await Promise.all([connection.getProgramAccounts(PROGRAM_ID, { filters: [{ dataSize: 200 }] }), readConfig(connection), connection.getMinimumBalanceForRentExemption(72)]); const next=rows.map(r => parseMarket(r.pubkey, Buffer.from(r.account.data))).sort((a,b)=>b.resolveTs-a.resolveTs); setMarkets(next); setOracle(configured.oracle); setAuthority(configured.authority); setFees(BigInt(Math.max(0,configured.lamports-rent))); if(wallet.publicKey){ const values=await Promise.all(next.map(m=>readPosition(connection,m.address,wallet.publicKey!))); setPositions(Object.fromEntries(next.map((m,i)=>[m.address.toBase58(),values[i]]))); } else setPositions({}); }, [connection,wallet.publicKey]);
  useEffect(() => { void refresh(); }, [refresh]);
  useEffect(() => { const timer = window.setInterval(() => setNow(Math.floor(Date.now()/1000)), 1000); return () => window.clearInterval(timer); }, []);
  async function send(ix: TransactionInstruction, label: string) { if (!wallet.publicKey || !wallet.sendTransaction) return; try { setBusy(label); setNoticeSignature(""); const sig = await wallet.sendTransaction(new Transaction().add(ix), connection); await connection.confirmTransaction(sig, "confirmed"); setNotice("交易成功"); setNoticeSignature(sig); await refresh(); } catch(e) { setNoticeSignature(""); setNotice(e instanceof Error ? e.message : "交易失败"); } finally { setBusy(""); } }
  async function create() { if (!wallet.publicKey) return; const createdAt = Math.floor(Date.now()/1000); if (hasUnfinishedMarket(markets, createdAt)) { setNotice("当前已有未结束的 30 分钟市场，结束后才能创建下一场。"); return; } const { ix } = createMarketIx(wallet.publicKey, BigInt(Date.now()), BigInt(createdAt+BETTING_WINDOW_SECONDS), BigInt(createdAt+MARKET_DURATION_SECONDS)); await send(ix, "create"); }
  async function settle(m: Market) { if (!wallet.publicKey || !oracle || !wallet.publicKey.equals(oracle)) return; try { setBusy(`settle-${m.address}`); const response = await fetch(`https://api.open-meteo.com/v1/forecast?latitude=${m.lat}&longitude=${m.lon}&current=precipitation&timezone=Asia%2FShanghai`); if (!response.ok) throw new Error(`天气 API 返回 ${response.status}`); const body = await response.json() as { current?: { precipitation?: number } }; const mm = body.current?.precipitation; if (typeof mm !== "number") throw new Error("天气 API 未返回降水量"); await send(settleIx(wallet.publicKey, m.address, mm), `settle-${m.address}`); } catch(e) { setNotice(e instanceof Error ? e.message : "结算失败"); setBusy(""); } }
  const isOperator = !!wallet.publicKey && !!oracle && wallet.publicKey.equals(oracle);
  const isAdmin = !!wallet.publicKey && !!authority && wallet.publicKey.equals(authority);
