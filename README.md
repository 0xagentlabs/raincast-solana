...
Solana Devnet 天气预测市场 Demo。用户可选择上海、北京、广州、深圳、成都、杭州、武汉或西安，预测未来 30 分钟内是否达到 0.1 mm 降水；Open‑Meteo 数据由受控预言机签名并用于链上结算。每个市场开放预测 25 分钟、在第 30 分钟结算，页面和链上 schedule PDA 都会阻止创建时间重叠的市场。

结算时平台收取奖池的 1%，其余资金全部按份额分配给获胜方；管理员可在后台一次领取累计平台收益，用户可在每个市场查看自己的 YES/NO 份额与领取状态。
